...
func agentRoleCardManualWithProfile(companyName, workspace string, rule AgentRule, profile roleManualProfile) []byte {
	...
	fmt.Fprintln(&b, "14. 若收到 `HQ守卫` 或 `HQ执行守卫` 通知，说明你已结束回合但 ledger 仍有超时的 durable 待办；立即执行通知给出的精确 HQ 命令并收敛原 case。专业员工完成后必须在原 assignment report，经理必须审查/推进自己的队列。经理已正式委派的 descendant assignment 在执行期间由员工执行守卫负责，不会同时催经理提前 report；child submitted 后才转为经理 review，收敛后父任务按最近业务 transition 重新计时。HQ 会有界重试并沿 reports_to 升级，但不会替任何人 report、accept/return 或伪造业务结论。")
	if isManagerResponsibilities(rule.Responsibilities) {
		fmt.Fprintln(&b)
