...
func agentRoleCardManualWithProfile(companyName, workspace string, rule AgentRule, profile roleManualProfile) []byte {
	...
	fmt.Fprintln(&b, "4. 只有正式 issue 才用 `hq accept --event <issue-event-id> --next <下一步>` 接单。Turn Bundle 中的 quiet handoff 是上下文，不会自行改变 case 状态；但收到并读懂 `question|request|handoff|directive` 后，必须按信封运行 `hq message ack --message <message-id>` 写入 durable ack。ack 只证明收到，不表示接受结论；普通 `info` 不要求 ack。")
	...
	fmt.Fprintln(&b, "7. 只有收到 return 才在原 assignment 下返工并重新 report；submission 一旦被 accept，旧 assignment 就已消费，后续 message 不能将它复活。再次工作必须等待经理发出的 fresh issue。执行中的旧合同若被 `assignment_superseded`，立即停止沿旧合同 accept/report，只接受信封中同一经理发出的 replacement assignment。只有冻结 reviewer 可以 accept 或 return 你的交付。")
	...
	fmt.Fprintln(&b, "13. 若收到 `[HQ runtime recovery]`，这是 HQ 在同一 seat 上恢复 durable 工作的信封，不是新任务。触发原因可能是模型载体切换，也可能是 active assignment 的 runtime 意外消失。信封只展开最多 8 项 actionable durable work；如提示仍有省略项，用 `hq inbox` 和 `hq assignment list` 读取完整队列。重读本手册、信封列出的 `assignment show`/`history` 和同一工位文件；只从 durable 状态继续，不臆造旧会话结论，不创建替代 case 或重复 assignment。")
	fmt.Fprintln(&b, "14. 若收到 `HQ守卫`、`HQ执行守卫` 或 `HQ加急指令守卫` 通知，说明你已结束回合但 ledger 仍有超时的 durable 待办；立即执行通知给出的精确 HQ 命令并收敛原 case。加急指令必须先阅读原信封再 ack，HQ 不会替你确认。专业员工完成后必须在原 assignment report，经理必须审查/推进自己的可执行队列。经理已正式委派的 descendant assignment 在执行期间由员工执行守卫负责，不会同时催经理提前 report；若 `hq issue` 返回 `actor_directive=end_turn`，经理必须立即结束回合，禁止 sleep、进程/Herdr 状态和产物轮询，等待 child submitted、blocked/needs-decision、投递异常或执行升级触发 HQ 唤醒。child submitted 后才转为经理 review，收敛后父任务按最近业务 transition 重新计时。HQ 会有界重试并沿 reports_to 升级，但不会替任何人 report、accept/return 或伪造业务结论。")
	fmt.Fprintln(&b, "15. `[HQ URGENT DIRECTIVE]` 是绑定当前 active assignment 的下一安全回合指令，不是硬中断，也不修改 objective/acceptance/constraints。读懂后立即 ack 并在当前合同内调整；若要求实际改变任务合同，只接受冻结 issuer 通过 `[HQ URGENT REVISION]` 发出的 replacement assignment。")
	if isManagerResponsibilities(rule.Responsibilities) {
		fmt.Fprintln(&b)
		...
		fmt.Fprintln(&b, "ceo-office/tools/hq/bin/hq message --to <direct-report-seat> --case <active-case-id> --kind directive --urgency urgent --text <当前合同内的加急指令>")
		fmt.Fprintln(&b, "ceo-office/tools/hq/bin/hq case revise --id <active-case-id> --version <N+1> --title <新标题> --objective <新目标> --acceptance <新验收> --constraints <新边界> --priority P0 --source <变更依据> --supersede-active --next <replacement第一步> --note <变更原因>")
		...
		fmt.Fprintln(&b, "`issue` 会建立 durable Assignment Contract，并在 durable issue intent/prepared 时立即预占目标 WIP，防止并发超卖；员工后续 `accept` 是确认接单，不是容量开始计数的时点。目标为 `on_assignment` 且当前休眠时，HQ 会通过 Herdr 自动 cold-resume 并递送门铃；经理不需要、也不得另发 `herdr prompt` 或执行 `hq up <on_assignment-seat>`。若 CLI 自动更新等原因使新 runtime 在接单前退出，HQ 会拒绝把短暂启动当成可投递 seat；对于已记为 sent 但仍未 accept 的原 assignment，watchdog 会收敛旧 session、cold-resume 同一 seat 并有界重投同一 payload，不创建第二份任务。这里禁止的是业务派工/激活绕行，不禁止人类所有者或其明确授权代理在 seat 已由 HQ 激活后，以 Herdr prompt 补充精确外部工具权限；该提示必须继续原 assignment，不能改变任何业务合同。员工交付被验收后，无未决工作的 runtime 可在有界 keep-warm 后自动休眠；seat、角色卡和工位仍在。若 issue 因 `hibernate_attempting`/`hibernate_unknown` 被拒绝，在任何 origin/WIP 写入前就已终止；使用上面的 status 确认同一 incarnation，再严格按报错指示单 seat retry，不得用裸 prompt 绕过。跨部门修复完成后的 message 只传上下文；对原 case 先 revise 到新版本再 fresh issue，不能要求旧 assignment 再次 report。执行中的临时指令使用绑定 active assignment 的 urgent directive；目标/验收/边界发生变化时只能由冻结 issuer 使用 `case revise --supersede-active`，它原子作废旧合同并给同一直属员工发送 replacement，不得越级或改投。")
		fmt.Fprintln(&b, "\n经理只有在存在可执行 durable 队列时，才不能用结束 Agent 回合代替队列收敛。gateway 会在你处于 idle/done 且 durable submission、经理本人 active assignment 或新建但尚未委派的 owned open case 超时时发送 `HQ守卫` nudge；accepted、blocked、escalated 等 case 不会仅因 owner 仍是你而触发。队列固定按待审 submission、经理本人 active/rework assignment、未委派 owned open case 排序，并只在同类内 FIFO；旧 open case 不会遮蔽验收或返工。若你在自己的 accepted/rework 父 assignment 下已正式委派 descendant，open child 会成为你的委派项，未消费的 child assignment 由员工的 activation/progress 守卫推进，父项不会同时催你提前 report。`hq issue` 会在成功后重新计算你的队列：返回 `actor_directive=continue_queue` 时按精确下一步继续；返回 `actor_directive=end_turn` 时说明只剩下属执行责任，必须立即结束本回合，禁止 sleep、进程/Herdr 状态或产物轮询，等待 HQ 在 submission、blocked/needs-decision、投递异常或执行升级时重新唤醒。child submitted 后成为你的 review 项。child 收敛后，父项按最近一次真实业务 transition 重新开始 stall 计时；普通 message 或 runtime 事件不能伪造进展。相同队列 basis 只做有界提醒，随后沿 registry `reports_to` 升级。收到后必须执行其中的 accept/return/report/case 命令；系统本身绝不代做验收。")
	} else if rule.hasResponsibility(roleApprovalWitness) {
		fmt.Fprintln(&b)
		...
		fmt.Fprintln(&b, "9. **加急需求变更**：当前合同内的临时指令用 `message --kind directive --urgency urgent` 发给直属经理；它只在下一安全回合投递并要求 ack，不修改合同。objective/acceptance/constraints 等重大变化必须由你对直属经理的 active root/workstream 执行 `case revise --supersede-active`；经理 accept replacement 后，再由经理对相应 child 和直属员工执行同样修订。不得越级直接修改普通员工合同。")
	}
...
func companyAgentHandbook(plan initPlan) []byte {
	...
	fmt.Fprintln(&b, "Gateway 自动检测 config.yaml 保存（250ms 防抖、每分钟补偿核对），独立应用员工模型变更；忙碌席位每 5 秒重试安全边界，不打断 working|blocked，不唤醒离线员工，on_drift=report 仍只报告。Agent 继续用获授权的 staff update，不直接编辑配置绕过在途任务或权限门禁。无效配置不会触发切换，按 gateway 错误修正后保存并运行 hq doctor --json。")
	fmt.Fprintln(&b, "用 `hq staff get --name <seat> --live --json` 只读核验 expected_runtime_profile 与 runtime_profile_status.observed/state/reason；waiting_idle 是等待回合结束，next_activation 是下次激活生效，unverified 不是成功，recovery_pending 是模型已匹配但任务恢复未完成。默认不带 --live 只查配置/WIP，not_checked 不代表模型已应用。恢复式重启可能改变 tab/session ID，但 seat、角色卡、工位与 assignment 保持不变；不要裸 Herdr 重启或因等待而降级模型。")
	...
	fmt.Fprintln(&b, "```")
	fmt.Fprintln(&b, "\n员工只接受 HQ issue。`[HQ message]` 和 Turn Bundle 是有引用的上下文，不会建立 assignment；`question|request|handoff|directive` 信封要求接收者读懂后执行 `hq message ack --message <message-id>`，ack 只证明收到，普通 `info` 无需 ack。urgent directive 绑定当前 active assignment、在下一安全回合递送，不会硬中断或修改合同；未 ack 会被 durable 催办并沿 reports_to 升级。`[HQ URGENT REVISION]` 才代表旧合同已被原子作废，员工必须核对并 accept replacement。未知投递状态按 delivery 恢复协议处理，不得另发一份业务命令。`[HQ runtime recovery]` 只恢复同一 seat 的 actionable durable 工作并最多展开 8 项；省略项通过 `hq inbox`/`hq assignment list` 查询。经理信封中的 `SUPERVISED_ASSIGNMENT` 表示其仍负有监督或后续 review 责任，但下属继续持有执行权，经理不得接管或重复委派。接收者必须按信封列出的查询命令重建上下文，不得重复接单或创建替代 case。若原 assignment 仍为 accepted/rework 而员工已 idle/done，gateway 会以 `HQ执行守卫` durable nudge 要求其继续并 report；runtime 消失时可 cold-resume 同一 seat。人工 nudge 仍不能对普通员工派工。`on_assignment` 员工在交付终态、有界 keep-warm 到期且无未决工作后可自动休眠；休眠只关闭当前 Herdr runtime，不删除角色卡、工位、seat 或业务历史，下一次 issue 会自动复用该 seat。")
	fmt.Fprintln(&b, "\n组织不依赖模型自觉维持队列。gateway 会对 idle/done 的专业员工核对 accepted/rework assignment，并对经理或总部联络职责位核对 durable submission、active assignment 和新建但尚未委派的 owned open case；accepted、blocked、escalated 等 case 不会仅因 owner 仍是经理而触发。经理已正式委派的 descendant 在执行期间由员工守卫负责，父 assignment 不重复催报；`hq issue` 根据最新 durable 队列返回 `continue_queue` 或 `end_turn`，后者要求经理停止轮询并等待事件唤醒。patrol 会把没有可执行经理动作、只有下属执行责任却持续 working 的 runtime 报告为 `manager_busy_without_action`，但不会冒险自动中断。child submitted 后进入经理 review，收敛后父任务按最近业务 transition 重新计时。另有销账守卫只把 `accepted|finding_accepted`、无活动合同/未决投递且 child 均已 closed 的后序候选提醒给唯一 `account_closer`；绝不选择 open/blocked/needs_decision。超时后发送带精确命令的 durable nudge并有界重试。所有守卫只促使责任人收敛，不会自动 report、accept/return/close、改变 owner/status 或作质量判断。")
	...
	fmt.Fprintln(&b, "\n执行中的临时加急要求使用 `message --kind directive --urgency urgent`：HQ 将它绑定 active assignment，投递到下一安全回合并要求 durable ack；它不改变任务合同。若目标、验收或边界改变，冻结 issuer 必须运行 `case revise --supersede-active`，让旧 assignment 永久失效并向同一直属 assignee 发 replacement。总部联络职责位只修订直属经理合同；经理 accept 后再修订 child 并推送直属员工，禁止越级。")
