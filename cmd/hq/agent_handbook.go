...
func companyAgentHandbook(plan initPlan) []byte {
	...
	fmt.Fprintln(&b, "```")
	fmt.Fprintln(&b, "\n员工只接受 HQ issue。`[HQ message]` 和 Turn Bundle 是有引用的上下文，不会建立 assignment；`question|request|handoff` 信封要求接收者读懂后执行 `hq message ack --message <message-id>`，ack 只证明收到，普通 `info` 无需 ack。未知投递状态按 delivery 恢复协议处理，不得另发一份业务命令。`[HQ runtime recovery]` 只恢复同一 seat 的 actionable durable 工作并最多展开 8 项；省略项通过 `hq inbox`/`hq assignment list` 查询。经理信封中的 `SUPERVISED_ASSIGNMENT` 表示其仍负有监督或后续 review 责任，但下属继续持有执行权，经理不得接管或重复委派。接收者必须按信封列出的查询命令重建上下文，不得重复接单或创建替代 case。若原 assignment 仍为 accepted/rework 而员工已 idle/done，gateway 会以 `HQ执行守卫` durable nudge 要求其继续并 report；runtime 消失时可 cold-resume 同一 seat。人工 nudge 仍不能对普通员工派工。`on_assignment` 员工在交付终态、有界 keep-warm 到期且无未决工作后可自动休眠；休眠只关闭当前 Herdr runtime，不删除角色卡、工位、seat 或业务历史，下一次 issue 会自动复用该 seat。")
