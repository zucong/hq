...
func TestDocumentationContractNamesFormalReleaseArtifact(t *testing.T) {
	...
	release := string(raw)
	for _, want := range []string{"v1.2.5 配置写请求排队", "v1.2.4 维护循环与配置写入活性", "v1.2.3 员工模型覆盖", "/tmp/hq-v1.2.5-release", "v1.2.2 销账空闲边界重武装", "v1.2.1 投递活性修复", "v1.2.0 加急在途变更", "v1.1.5 事件驱动的经理停车"} {
...
func TestDocumentationContractPublishesQueuedActionRecovery(t *testing.T) {
	...
	for path, wants := range contracts {
		...
		for _, want := range wants {
			if !strings.Contains(string(raw), want) {
				t.Fatalf("%s missing queued-action recovery contract %q", path, want)
			}
		}
	}
}
