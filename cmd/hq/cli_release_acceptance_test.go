...
func TestCLIPureParameterContractsFailBeforeOfficeDiscovery(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "active revise requires next", args: append(append([]string{"case", "revise"}, caseSpec...), "--version", "2", "--supersede-active"), want: []string{"replacement assignment", "--next TEXT"}},
		{name: "ordinary revise rejects next", args: append(append([]string{"case", "revise"}, caseSpec...), "--version", "2", "--next", "work"), want: []string{"--next 仅与 --supersede-active"}},
		...
		{name: "directive requires case", args: []string{"message", "--to", "worker", "--kind", "directive", "--text", "work"}, want: []string{"--case CASE_ID", "case revise --supersede-active"}},
		{name: "urgent requires directive", args: []string{"message", "--to", "worker", "--kind", "request", "--urgency", "urgent", "--text", "work"}, want: []string{"只允许与 --kind directive", "active assignment"}},
		{name: "urgent rejects quiet", args: []string{"message", "--to", "worker", "--case", "CASE-PARAMETER-AUDIT", "--kind", "directive", "--urgency", "urgent", "--delivery", "quiet", "--text", "work"}, want: []string{"下一安全回合", "--delivery wakeup"}},
